...
and ```catplot``` it's like ```relplot``` but used to create categorical plots <br/>
**What is a box plot?**<br/>
* used for quantitative data
* facilitates comparisons between groups

if we want to remove outlier we use `sym` paramater
```
sns.catplot(x='',y='',data='',kind='',sym='')
```
and if you want to customize the line out of in box plot we use paramater
`whis`
```
sns.catplot(x='',y='',data='',kind='',sym='',whis=IGR)
```
if you want to make it from range to range
```
sns.catplot(x='',y='',data='',kind='',sym='',whis=[0,95])
```
![alt text](image-29.png)

**what is the point plot**
* It show mean of quantitative variable
* Vertical line show 95% confidence intervals 
* has categorical variable on x-axis

![alt text](image-30.png)
paramaters<br/>
* `join` it is used to connect or disconnect the point 
* estimator=function-like median-

We can define the color of plot by 
```
sns.set_palette('List of palette')
```
There's Type of color in sns:
1. Diverging Palettes
![alt text](image-31.png)
2. Sequential Palettes<br>
It's great for emphasizing a variable on a **continuous scale** 
![alt text](image-32.png)

if the scale of the plot we want to change we use ```sns.set_context()```
paper->notebook->talk->poster
<br>
to change style of plot ```sns.set_style()```<br>
in seaborn we have 2 types of object (Facetgrid,AxesSubplot)
<br> 
![Facetgrid](image-33.png)
Facetgrid can create subplot like relplot and catplot <br>
AxesSubplot can't create subplot like scatterplot and countplot
___
...
if we create a fucntion and dont know what is the input will be we can use ```*arg```
```
def add_all(*args):
    ...
    return sum_all
```
what is the difference between ```*arg``` and ```**kwargs```?<br>
```*arg``` for tuple 
```**kwargs``` for dictionary

``map`` & `filter` give the address of list not the list we need to get the list by ```list(address)``` not like reduce return the normal out of lambda
```
...
# Python Toolbox
**Iterables:** <br>
An iterable is any Python object you can loop over.<br>
Like list,strings,dictionaries,file connection `iter()` <br>
**Iterators:<br>**
It's a object produce next value the `next()`
```
word='da'
it=iter(word)
print(next(it)) # d
...
print(*it) # da
```
***an iterable is an object that can return an iterator, while an iterator is an object that keeps state and produces the next value when you call next() on it***<br>
enumerate return tuple index and value<br>
zip make a iteratare of tuple we put index and value to get tuple
![alt text](image-34.png)
**enumerat** return tuple with index <br>
**zip** combined two or more iterable tuple